...
- [ ] T-015b2a: `signal-gateway` primitive wiring — `app/metrics.py` expansion (`Metrics` dataclass + `build_signal_gateway_metrics`, four §15.3 metrics: `signals_received_total{source}`, `signals_validated_total{status}`, `errors_total{service, error_class}`, `webhook_processing_seconds`), `app/models.py` (`SignalEnvelope` with `model_validator(mode="before")` migrating unknown top-level keys into `payload`; `WebhookValidatedResponse` / `WebhookDuplicateResponse` / `WebhookErrorResponse`), `app/middleware.py` (`RateLimitMiddleware` wrapping T-015b1 `RateLimiter`; client-IP from `X-Real-IP` / `X-Forwarded-For` / `request.client.host`; observes `webhook_processing_seconds` end-to-end on `POST /webhook` only), `app/deps.py` expansion (six new providers: `get_settings`, `get_rate_limiter`, `get_dedup`, `get_symbol_map_cache`, `get_metrics`, `get_trading_logger`), `app/main.py` lifespan attaches `settings` + primitives + metrics + trading logger to `app.state` and registers `RateLimitMiddleware` (no `include_router` for webhook yet). `test_app_factory.py` extension verifies state attachments + middleware presence + `/webhook` still 404. Spec: §9.1, §15.3, §16.3, §N6, §N7, §0.8. Hazards addressed: H-006 (rate limit).
- [ ] T-015b2b: `signal-gateway` `/webhook` handler — `app/webhook.py` orchestrates the §9.1 13-step pipeline (raw body → HMAC → `signals.raw` publish → JSON parse → idempotency_key peek → Pydantic validate → `signals_received` increment → `bind_correlation` → dedup → symbol resolve → `insert_signal` → `signals.validated` publish with `message_id_for` → 200), `app/main.py` `include_router(webhook_router)`, `tests/test_webhook.py` (per-branch unit tests with mocked deps), `tests/integration/{conftest.py, test_webhook_e2e.py}` (env-gated `POSTGRES_TEST_DSN` + `NATS_TEST_URL`; happy path + duplicate edge), module doc `*future (T-015b2)*` markers flipped to present tense plus new "Pipeline wire order" + "Response codes" + "Log event catalog" sections. Spec: §9.1, §5.7, §8.4, §15.3, §19 F0 bullet 8. Hazards addressed: H-010 (fan-out before dedup).
