"""Liveness and readiness routes for analytics-api (§9.6, §18.4).

Two endpoints, both owned by the T-400 skeleton:

* ``GET /health`` — liveness. Returns ``200 {}`` whenever the process
  can handle a request. No dependencies, cannot fail while the service
  runs. Docker healthcheck target per §18.4. Mirrors strategy-engine
  / execution-service / feature-engine / market-data-svc / signal-
  gateway verbatim.
...
* ``GET /ready`` — readiness. Returns ``200 {"ready": true}`` when
  **both** of (a) :class:`packages.bus.NatsClient` is ``CONNECTED`` and
  ...
  ``reason="db"`` key matches T-100/T-109/T-214/T-309 verbatim so
  monitoring alert rules work uniformly across services. Distinct from
  ``/health`` so the container stays up (still emitting JSON logs,
  still exposing ``/metrics``) while a downstream dep is transiently
  unavailable.

T-400 ships ``bus`` + ``db`` reasons only. analytics-api skeleton has
no other lifespan deps; future endpoint groups (T-401..T-408) read
from PG / NATS through the same bus + pool, so no third readiness
reason is added.
"""
