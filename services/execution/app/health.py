"""Liveness and readiness routes for execution-service (§9.5, §18.4).

Two endpoints, both owned by the T-214 skeleton:
...
* ``GET /health`` — liveness. Returns ``200 {}`` whenever the process
  ...
  market-data-svc / feature-engine verbatim.
...
* ``GET /ready`` — readiness. Returns ``200 {"ready": true}`` when
  **both** of (a) :class:`packages.bus.NatsClient` is ``CONNECTED`` and
  ...
  ``reason="db"`` key matches T-100/T-109 verbatim so monitoring alert
  rules work uniformly across services. Distinct from ``/health`` so
  the container stays up (still emitting JSON logs, still exposing
  ``/metrics``) while a downstream dep is transiently unavailable.

T-214 ships ``bus`` + ``db`` reasons only. **T-209 is expected to add
a third reason key (likely ``exchange_ws``) when the private Bybit WS
connection lands** — readiness should flip 503 if the private WS is
disconnected because order-event dispatch (T-218) depends on it.
That extension is owned by T-209's plan-doc; the T-214 contract does
not pre-design the third reason per §0.8 anti-hypothetical.
"""
