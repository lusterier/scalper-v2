"""Shared fixtures for execution-service unit tests.
...
* :func:`packages.db.create_pool` is monkey-patched at the
  ``services.execution.app.main`` boundary to return a mock pool.
* :class:`packages.bus.NatsClient` patched similarly.

The lifespan then runs end-to-end against the mocks, attaching pool
and bus to ``app.state`` exactly as it would in production. Tests
mutate ``mock_bus.state`` / pool-acquire behaviour to simulate
outages for the ``/ready`` coverage matrix.

T-214 ships the skeleton: pool + bus only. Downstream tasks (T-215
adapter pool, T-218 dispatcher, T-220 scheduler, T-221 reconciliation)
will extend the lifespan composition; their plan-docs add fixtures
here at first import.
"""
