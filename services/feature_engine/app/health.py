...
Two endpoints, both owned by the T-109 skeleton:
...
* ``GET /health`` — liveness. Returns ``200 {}`` whenever the process
  ...
  / market-data-svc verbatim.

* ``GET /ready`` — readiness. Returns ``200 {"ready": true}`` when
  **both** of (a) :class:`packages.bus.NatsClient` is ``CONNECTED``
  and (b) an asyncpg pool connection acquires inside 1 s. Otherwise
  ``503 {"ready": false, "reason": <bus|db>}``. Reason precedence is
  bus → db (cheapest check first; first failing reason wins). The
  ``reason="db"`` key matches T-100's market-data-svc verbatim so
  monitoring alert rules work uniformly across services. Distinct
  from ``/health`` so the container stays up (still emitting JSON
  logs, still exposing ``/metrics``) while a downstream dep is
  transiently unavailable.

No ws-equivalent — feature-engine has no analogous long-lived
external connection beyond bus + pool. T-110's dispatcher reuses the
bus; the readiness contract does not change when T-110 lands.
"""
