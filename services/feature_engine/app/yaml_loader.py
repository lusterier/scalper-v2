...
Replaces T-110c's hardcoded :func:`build_features` demo with a config-as-data
loader that reads ``configs/features/indicators.yaml`` (§B.2 verbatim shape)
and cross-products each indicator with the symbol set
...
T-111 is BUILTINS-only YAML registration.
"""
...
def load_indicators_yaml(
    ...
    for entry in raw["features"]:
        ...
        for symbol in symbols:
            ...
            # `ind.{symbol}.15m.ema_20` — interval baked in, only `{symbol}`
            # remaining). Feature.name_template has 2 placeholders
