...
## L-011 (F4 E1 smoke 500, post-hoc fix on master, 2026-05-07)
Pattern: `json.dumps(state_dict)` without `default=str` raises `TypeError` whenever the dict contains UUID / datetime / Decimal — but mock-based tests on `conn.fetchrow` capture the encoded bytes and don't exercise the encoder failure mode. T-407 / T-415 ship `_audit_state_dict(BacktestRunRow)` projecting a dataclass with UUID id + TIMESTAMPTZ datetime fields; existing `packages/db/tests/test_queries_audit.py` only ever passed plain string-keyed dicts as `before_state`/`after_state`, so the json-encoder gap surfaced live during F4 E1 dashboard smoke when `POST /api/backtests/` returned 500 ("Object of type UUID is not JSON serializable"). Sibling `packages/db/queries/execution.py:202` already used `default=str`; the audit helper drifted from that precedent.
Active control: When a query helper invokes `json.dumps(...)` on a payload that may contain UUID / datetime / Decimal — i.e. any value sourced from a dataclass row projection or `asyncpg.Record` — brief-reviewer MUST verify `default=str` is set. At plan-reviewer stage, any plan that introduces a `_*_audit_state_dict` / `_*_payload_dict` helper feeding `insert_audit_event` (or any JSONB column writer) must include a regression test that constructs a state dict containing UUID + datetime + Decimal and asserts the helper succeeds (not just `json.loads` round-trip equals input). Mock-based tests are still acceptable for SQL-shape pinning, but the JSON-serialise path must be exercised with real types — dropping `default=str` should re-fail the test, not silently pass.
