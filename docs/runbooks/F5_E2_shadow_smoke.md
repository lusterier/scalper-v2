...
- [ ] **Honesty note (L-028 / T-522 close-out decision A):** the individual D-fixes in this runbook are close-out-RUN-verified-correct (commit `b16d41a` / `docs/status.md`), but the full F5_E2 live signal→kill→restart sequence was **NOT re-run end-to-end under T-540**. The `strategy-engine-smoke` service **ships as of T-544** (the service-missing blocker is removed) — the full deployment smoke is now runnable once the D7 (`bots`-row INSERT) / D10 (`BOT_SMOKE_PAPER_*` env) / `ohlc_1m`-seeding prerequisites above are satisfied. **E3's sanctioned verification is CI-grade** (the 2 controlling restart tests green in CI-full + the T-519 §20 audit + the E4 hazard meta-test); this deployment smoke is the optional deeper confirmation, NOT the E3 sign-off gate.
