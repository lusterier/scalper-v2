...
**Test.** `test_tp_size_below_min_order_qty_rejects_before_place` + `test_tp_size_qty_step_round_down_proceeds_with_aligned_size` + `test_tp_qty_pct_one_boundary_proceeds`.
...
**Policy.** Every SL/TP/BE/trail price MUST be passed through `quantize_price(price, side, instrument_info.tick_size)` (T-558a; `buy`→ROUND_FLOOR / `sell`→ROUND_CEILING — SL never tighter, TP never further) BEFORE `set_trading_stop`, and the SAME quantized Decimal MUST be the value persisted (`persist_placement_tx` / `position_state` / BE-trail SL updates) so DB == exchange. In `placement.py` the quantize is applied once at the SL/TP compute site, BEFORE the paper/live fork, so paper, live, and persist receive the identical quantized value (parity preserved). (placement SL/TP: T-558b1; lifecycle BE/trail + trail_audit `expected`: T-558b2 — both DONE.)

**Test.** `test_placement_sl_tp_tick_quantized_at_set_trading_stop_and_persist` + `test_placement_quantize_price_uses_request_side_and_instrument_tick` + `test_lifecycle_be_sl_tick_quantized_at_set_and_persist` + `test_trail_audit_expected_quantized_no_false_drift`.

H-038 numbering note: F6 triage-finding-#2 (surfaced by the T-556 live-diagnostic 2026-05-18; operator-prioritized ahead of triage C as direct capital risk). Sibling-class of H-037 (same instrument-filter capital-safety discipline as the qty side) but a distinct failure mode (price tick vs partial-TP min-lot), distinct origin, distinct policy (quantize-and-persist vs reject). Placement SL/TP enforced + tested in T-558b1; lifecycle BE/trail + trail_audit `expected` enforced + tested in T-558b2 (this same block — finding #2 fully closed). Not in the H-030..H-036 closed audit cluster.
