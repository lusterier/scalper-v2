...
## 2026-05-18 session (T-557 hazcov hotfix + T-558a — finding #2 foundation; push-gate lifted)

**Push-gate lifted:** operator authorized "mozes spravit push" → master pushed (`454196f..50a6f96`, then the `fix(T-557)` hazard-catalog hotfix `80a6bf8`). **T-557 follow-up:** the H-037 §20 `**Test.**` block cited a bare `test_placement.py` filename → repo-root `tests/test_hazard_catalog_coverage.py` (`_PATH_RE` = file-citation needing the full collected path) red in CI `unit` post-push (all 4 T-557 gates + `services/execution/` suite were green — that meta-test is repo-root, not in the touched-service suite). Hotfix `80a6bf8`: cite the 3 T-557 func-names (mirror H-032). **NEW L-036** (§20-edit must cite func-name/`path::func` + run the repo-root hazard-catalog meta-test in pre-push).

**F6: 16/17 numbered done (T-542..T-558a; T-558b queued).** **T-558a** (triage finding #2 FOUNDATION; operator pre-emptive split L-007) shipped: `feat 2199278` + `chore` (this commit). NEW `InstrumentInfo.tick_size` (required) + `quantize_price(price, side, tick)` (buy→floor / sell→ceil, side-aware conservative per OQ-1) + bybit/paper `get_instrument_info` parse `priceFilter.tickSize`; 12 `InstrumentInfo(` sites updated in-commit (L-034). **NO behavior change** (no caller — T-558b applies). Gate-1 pass-1 caught a real capital bug in the plan: `Decimal //` truncates toward zero (NOT floor toward −∞) — the original ceil formula would have shipped a SHORT-side capital regression; corrected to `to_integral_value(ROUND_FLOOR/ROUND_CEILING)`. **NEW L-037** (Decimal `//` truncate-toward-zero; recompute the formula not just the expected value). math-validator EXPLICITLY invoked despite path-trigger OOS (adjudication b) — 9 hand-cases + 400k-sample Decimal stress, 0 mismatch. NO §20 in T-558a (the H-037-sibling hazard binds at the T-558b apply-site — avoids a premature L-036 obligation). `quantize_qty`'s false `//`-floor docstring sentence bound-out (§0 anti-absorb → F6+ trivial `docs(...)`).

**Triage queue (operator-prioritized; #1=T-557 DONE):** **#2** in progress — **T-558a foundation DONE, T-558b (APPLY) is the next actionable** (apply quantize_price at placement SL/TP + lifecycle BE/trail + persist quantized [kills the T-557-OQ-3-deferred §N1 DB↔exchange drift] + trail_audit sibling + NEW §20 H-037-sibling hazard [cite func-names + run hazard-catalog meta-test per L-036]). Then **#3** (`latest_price` KV missing) → **C** (dispatcher H-031) → **T-551** → **T-555** → F6+ doc-audit (+ runbook webhook-contract drift + `quantize_qty` false-docstring trivial doc-fix). **Next-session pickup = T-558b.**

**Stack state:** unchanged — execution-service container = T-554 build; T-556/T-557/T-558a all committed to master but **NOT deployed** (rebuild+recreate needed to arm the demo bot; not needed until operator re-arms). demo bot `status=paused`/`demo`, Bybit demo flat, stack healthy. ORDERS_DLQ provisioned (T-553); no nats-init re-run until T-555.

**Git:** T-556/T-557(+hotfix) pushed; T-558a `feat 2199278` + this `chore` pushed at this close (operator-authorized — "mozes spravit push"). origin == local master. `.claude/settings.json` still operator-parked (decision B; never staged — all-session discipline held across T-556 / T-557 / T-558a).

---

## 2026-05-18 session (T-558b1 — finding #2 APPLY part 1; placement SL/TP tick-quantized)

**F6: 17/18 numbered done (T-558b1 DONE; T-558b2 queued — next actionable).** Operator pre-emptive split L-007: parent T-558 → T-558a (foundation, DONE) → T-558b further split (operator 2026-05-18, on the ~120+ price-assert / 8-test-file / 3-subsystem blast-radius) → **T-558b1** (placement, DONE `feat 1f1ecf9`) + **T-558b2** (lifecycle BE/trail + trail_audit, queued).

**T-558b1 shipped:** `placement.py:435-436` `compute_sl_price`/`compute_tp_price` wrapped in `quantize_price(_, request.side, instrument_info.tick_size)` BEFORE the paper/live fork → paper+live+`persist_placement_tx` all get the SAME exchange-valid quantized Decimal (parity preserved; DB==exchange → kills the §N1/reconciliation drift, the T-557-OQ-3-deferred concern). buy→ROUND_FLOOR / sell→ROUND_CEILING (SL never tighter / TP never further). NEW §20 **H-038** (sibling-class H-037; full SL/TP/BE/trail policy + `(placement: b1; BE/trail: b2)` parenthetical, L-026 non-partial; `**Test.**` cites bare func-names per L-036 — the T-557 hazcov lesson held, hazard-catalog 8 passed pre-push). All 4 gates (plan-reviewer APPROVE pass-2/2 — pass-1 = 2 sibling-enumeration BLOCKERs resolved by grep-evidence; drift ON TRACK; brief SHIP 5/5; math-validator VERIFIED ACTIVE — SL 44775.4/TP 45675.5/sell-SL 45225.6 independently recomputed, capital-safe, no SHORT inversion). repo-wide 2634 passed/0 failed. NO new ADR / NO new review-lesson. Notable: Gate-1 pass-1 caught my over-broad sibling-enumeration (claimed "placement/persist tests"; reality `test_placement_persist.py` ×12 is byte-unchanged carve-out — T-558b1 changes neither `compute_*` nor `persist_placement_tx`; actual changed-set = EXACTLY 4 asserts in `test_placement.py`) — L-034 enumeration-precision, grep-evidenced fix.

**Triage queue (operator-prioritized; #1=T-557 DONE):** **#2 in progress** — T-558a foundation DONE, T-558b1 (placement) DONE, **T-558b2 (lifecycle BE/trail + trail_audit) is the next actionable** (apply `quantize_price` at `_compute_be_sl_price`/`_compute_trail_sl_price` → set_trading_stop + persist, tick via `adapter.get_instrument_info(symbol).tick_size` cached; trail_audit `expected` quantized + keep the existing drift-tolerance; extend §20 H-038 `**Test.**` to append BE/trail func-names; update the resolved `lifecycle.py:47` limitation-comment; shadow_worker bound-out). Then **#3** (`latest_price` KV missing) → **C** (dispatcher H-031) → **T-551** → **T-555** → F6+ doc-audit (+ runbook webhook-contract drift + `quantize_qty` false-`//`-docstring trivial doc-fix). **Next-session pickup = T-558b2.**

**Stack state:** unchanged — execution-service container = T-554 build; T-556/T-557/T-558a/T-558b1 all committed to master but **NOT deployed** (rebuild+recreate needed to arm the demo bot; not needed until operator re-arms). demo bot `status=paused`/`demo`, Bybit demo flat, stack healthy. ORDERS_DLQ provisioned (T-553); no nats-init re-run until T-555.

**Git:** T-558b1 `feat 1f1ecf9` + this `chore` pushed at this close (operator-authorized — "mozes spravit push" standing). origin == local master. `.claude/settings.json` still operator-parked (decision B; never staged — all-session discipline held across T-556 / T-557 / T-558a / T-558b1).

---

## 2026-05-18 session (T-558b2 — finding #2 FULLY CLOSED; F6 18/18)

**F6: 18/18 numbered done — ALL F6 numbered tasks complete.** **Triage finding #2 (no price/tick quantization for SL/TP/BE/trail) FULLY CLOSED** across the operator pre-emptive L-007 split chain: T-558a (foundation: `InstrumentInfo.tick_size` + `quantize_price` helper) → T-558b1 (placement SL/TP apply + persist + §20 H-038) → **T-558b2** (lifecycle BE/trail apply + persist + trail_audit `expected` + H-038 amended; `feat 31a9ddb` + this `chore`). All 4 gates clean (plan-reviewer APPROVE 2-CONCERNs→5-WG / drift ON TRACK / brief SHIP 5/5 / math-validator VERIFIED ACTIVE — values independently recomputed, side conservative non-inverted, the trail_audit no-false-drift test non-vacuous). NO new ADR / NO new review-lesson.

**T-558b2 substance:** `lifecycle.py` BE+trail quantize the `_compute_be/trail_sl_price` output side-aware (buy→FLOOR/sell→CEILING, never tighter) before set_trading_stop; the SAME quantized Decimal is persisted (`update_position_state_sl`) → DB==exchange, the T-557-OQ-3-deferred §N1 drift now fully resolved. `trail_audit.py` `expected` quantized identically (Phase B conn-free) — without it the now-quantized lifecycle trail SL would false-fire `trail_sl_drift_detected` on every trailing position; the drift tolerance is byte-unchanged (OQ-2). `lifecycle.py:44-49` OQ-3=A "Bybit normalizes SL to tick" limitation comment RESOLVED. §20 **H-038 amended in-place** (Test += 2 b2 func-names per L-036; BOTH forward-ref sites b2-done per L-026/L-035/WG#2; H-037/H-036/cluster byte-unchanged). Zero-ripple test design (mirror T-557): 2 sibling fixtures + `get_instrument_info` AsyncMock tick 0.001 → existing on-grid asserts byte-unchanged; 2 new hand-literal tests prove the apply (sub-tick fixture). H-024/H-013 NOT regressed; shadow_worker bound-out.

**Triage queue (operator-prioritized; #1=T-557 ✅ #2=T-558a/b1/b2 ✅):** **#3 next actionable** — `latest_price` NATS KV bucket not found (`BucketNotFoundError bucket=latest_price`, T-556 live-diagnostic) — missing-bucket/stream class (T-548/T-553 lineage; likely a `bootstrap.sh`/`infra/nats` provisioning gap + the consumer read-path). Not yet scoped — desk-investigate at pickup; becomes a numbered F6 task at its Gate-1 (ADR-0015 decision-C). Then **C** (dispatcher unattributable_fill / H-031) → **T-551** (cross-service NATS-consumer supervision) → **T-555** (nats-init re-apply idempotency) → F6+ doc-audit backlog (+ runbook webhook-contract drift + `quantize_qty` false-`//`-docstring trivial doc-fix). **Next-session pickup = finding #3.**

**Stack state:** unchanged — execution-service container = T-554 build; T-556/T-557/T-558a/T-558b1/T-558b2 all committed to master but **NOT deployed** (rebuild+recreate needed to arm the demo bot with all fixes; not needed until operator re-arms). demo bot `status=paused`/`demo`, Bybit demo flat, stack healthy. ORDERS_DLQ provisioned (T-553); no nats-init re-run until T-555.

**F6 status:** all 18 numbered tasks done (T-542..T-558b2). ADR-0015 exit-criteria were MET at T-544; T-545+ were operator-directed decision-C post-MVP fixes. Whether the F6 *phase* formally closes vs stays open for the remaining triage (#3/C/T-551/T-555) + F6+ opportunistic backlog is the operator's call (mirror the F5→F5+ precedent — phase-close is an operator act, not an author declaration).

**Git:** T-558b2 `feat 31a9ddb` + this `chore` pushed at this close (operator-authorized — "mozes spravit push" standing). origin == local master. `.claude/settings.json` still operator-parked (decision B; never staged — all-session discipline held across T-556 / T-557 / T-558a / T-558b1 / T-558b2).

---

## 2026-05-18 session (full stack redeploy + demo-trade reconciliation + triage-C repro consolidated; C re-prioritized → T-559)

**Full stack redeploy DONE (ops, no gates).** Operator authorized "komplet redeploy". 8 app images rebuilt+recreated at HEAD `e129705` via `docker compose up -d --build --no-deps analytics-api alerting execution-service feature-engine market-data-svc signal-gateway strategy-engine-demo strategy-engine-smoke` (T-556 proven-safe pattern; `--no-deps` → nats/postgres/nats-init/nginx/cloudflared untouched, no JetStream re-provision, no T-555 interaction). **No migrations** (DB `0021` == host head; F6 wave was code-only). All 13 services healthy. **Supersedes the prior "NOT deployed" stack-state note** — stack now == master; T-549a..T-558b2 all LIVE not just committed. Surfaced + fixed "failed to load bots": stale analytics-api image predated T-549a's `ExchangeMode.DEMO` → `/api/bots/` HTTP 500 (`ValueError: 'demo' is not a valid ExchangeMode` at `analytics.py:191`); post-redeploy `/api/bots/` → 200 with the demo bot row. Code was already correct (T-549a `bb62f79`); pure stale-image, no code change.

**Demo-trade reconciliation (operator-closed; DB correct, nothing lost).** Bybit demo trade-history showed TWO round-trips today; DB has ONE trade. Operator-confirmed explanation, corroborated by the DB evidence pulled this session:
- **Morning (Bybit `79a8544f` Open Long 06:04:04 UTC @ 76900 / `fd59458d` Close 06:09:02 UTC @ 76702):** signal `id=3` `manual-test-004` (`received_at` 06:04:03 UTC) DID validate + score + publish — `scoring_evaluations` id=2 (06:04:04, passthrough) + `outbox_events` id=3 (`signals.validated`, published 06:04:04.246) exist — but **ZERO execution-side rows** (no `orders`/`trades`/`trading_events`/`position_state`/`audit_events`). The dispatcher crashed **before** placement persistence (triage C, below): clean crash-before-persist, no partial write, no corruption, nothing deleted. This is the untracked SL-less Bybit position the operator closed manually.
- **Afternoon (Bybit `1a01ca38` Open 10:38:01 UTC @ 76785.40 / `b743f226` Close 10:44:36 UTC @ 76781.40):** signal `id=7` `t556-diag-1` (T-556 diagnostic) persisted correctly (`trades` id=1, `orders` id=1, 2× `trading_events`). Then deliberately closed via SQL `close_reason='t556_diagnostic'` → `exit_price=null`/`realized_pnl=0`/`lifecycle_state='open'` is the EXPECTED artifact of that manual diagnostic teardown (NOT a reconcile gap — earlier "T-221 backfill gap" finding WITHDRAWN per operator). DB == correct: 1 trade, 1 close, internally consistent. **No DB action taken** (trade-1 row left as the diagnostic record per operator "DB je správna").

**TRIAGE-C REPRO CASE (consolidated — code-cited + concrete evidence; for T-559 Gate-1).**
- *Mechanism:* `run_dispatcher_for_bot` (`services/execution/app/dispatcher.py:364-375`) consumes the exchange **WS** `adapter.stream_executions()` (NOT a NATS pull consumer — no JetStream ack/nak/redelivery on THIS path). Any `Exception` → `bound_logger.error("execution.dispatcher_stream_terminated")` + **re-raise** → that bot's dispatcher task dies (service stays up via lifespan `return_exceptions=True`; per-bot isolation by design T-216a WG#7/T-221, recovery only on service restart → T-221 reconcile). `_process` (`:192-200`): `order_id_match is None` AND `trade_id is None` → `raise RuntimeError("unattributable fill: no order match and no position_state")`. Triggered by the **WS-fill-before-persist race**: the open-fill WS event for a freshly-placed REST order arrives at `_process` before the `placement_persist` tx commits `orders`/`trades`/`position_state` → `select_order_id_by_exchange_id` None + `_derive_exec_type` → `("unknown", None, None)` (`:405`) → unattributable RuntimeError → `dispatcher_stream_terminated` → dispatcher dead → freshly-opened exchange position is **untracked & SL-less** until restart.
- *Concrete real-world repro (2026-05-18, demo bot):* signal `id=3`/`manual-test-004` 06:04:03 UTC → Bybit order `79a8544f` filled 06:04:04 UTC → WS fill beat `placement_persist` → `dispatcher_exec_type_unknown → dispatcher_unattributable_fill → dispatcher_stream_terminated` → 0 execution-side rows → operator manually closed the untracked position. (Per status 2193/2222 first-observed at T-552 verification; THIS session adds the Bybit-side order IDs/prices/times + the clean-no-partial-write confirmation.) Note BRIEF §20 **H-031 is a DIFFERENT hazard** ("paper adapter must not feed live dispatcher") — status uses "H-031" loosely as an area pointer; T-559 will allocate the correct §20 H-NNN at its apply-site.
- *Safety classification (operator):* verified real-world impact + capital-safety risk (untracked position with no SL) → operator re-prioritized C **ahead of #3**.

**Triage queue (operator re-prioritized 2026-05-18; #1=T-557 ✅ #2=T-558a/b1/b2 ✅):** **C = `T-559` — NEXT actionable** → **#3** (`latest_price` KV missing) → **T-551** (cross-service NATS-consumer supervision) → **T-555** (nats-init re-apply idempotency) → F6+ doc-audit backlog (+ runbook webhook-contract drift + `quantize_qty` false-`//`-docstring trivial doc-fix). T-559 is a development task → full Gate-1..4, numbered F6 task (ADR-0015 decision-C; per-item ADR only if plan-stage warrants — the retry-vs-skip-vs-DLQ + stream-termination-contract decision may warrant one). **Next-session pickup = T-559 (triage C) Gate-1.**

**Stack state:** ALL app services on HEAD `e129705` (redeployed this session). All 13 healthy. demo bot `status=paused`/`demo`, Bybit demo flat. ORDERS_DLQ provisioned (T-553); no nats-init re-run (used `--no-deps`).

**Session-end (T-559 Gate-1 DONE; parked clean):** Triage-C Gate-1 COMPLETE — plan-reviewer **APPROVE rev2** (rev1 REVISE → 2 BLOCKERs cleared, independently verified: BLOCKER-A `:192-200` raise-disposition baked into plan [KEPT defensive in-tx invariant + distinct `_post_gate` key, no reviewer-delegated decision]; BLOCKER-B L-026 broad-stem hit-set classified — 3 fix-in-commit [`dispatcher.py:357-362`+`:120-122` docstrings, `BRIEF:2926` H-031 Context xref] / 2 bound-out paper-domain [`pool.py:109`,`main.py:236`] / non-sites incl. T-216a startup fail-fast). Written: `docs/adr/0018-dispatcher-unattributable-retry-stream-survival.md` (Proposed), `docs/plans/T-559.md` (consolidated + 7 Write-time guidance verbatim), TASKS.md T-559 `[ ]` entry + F6 header (`18 done + T-559 plan-approved`; L-027 — counter bumps at close, 19/19 at T-559 close). **T-559 parked at *implementation pending operator "proceed"* — NO code written, nothing of it staged (clean pickup).** **Next-session pickup = T-559 implementation:** operator "proceed" → Gate-2 drift-checked TDD-first (6 hand-driven dispatcher tests, conn-free-backoff pin is load-bearing) → Gate-3 brief → Gate-4 math-validator path-triggered (`services/execution/` → expect VERIFIED out-of-scope, no financial math). Triage queue after T-559: **#3** (`latest_price` KV missing) → **T-551** (NATS-consumer supervision) → **T-555** (nats-init re-apply idempotency) → F6+ doc-audit backlog (+ runbook webhook-contract drift + `quantize_qty` false-`//`-docstring trivial doc-fix). **Open (separate, ops — NOT a dev task):** Vite boot-persistence `/tmp/scalper-ui.service` still needs operator `!`-sudo install (agent has no sudo); repo-commit of the unit (`infra/systemd/` + ops.md note) un-answered by operator. **Git:** session-end `chore` (TASKS+status+ADR-0018+plan/T-559) committed + pushed under the standing "mozes spravit push"; `.claude/settings.json` still operator-parked (decision B; never staged — discipline held ALL session incl. this chore).
